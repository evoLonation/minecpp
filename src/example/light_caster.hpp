...
int run(){
   try{
      ObservableValue type {LightType::DIRECTIONAL};
      ...
      // cube model
      std::vector<ObservableValue<glm::mat4>> cubeModels;
      for(int i = 0; i < 10; i++){
      ...
      // view, view pos
      ObservableValue viewModel {newViewModel(glm::vec3(3.0f, 0.0f, 3.0f))};
      ...
      // light info
      // directional
      ObservableValue lightDirection { glm::vec3(-3.0f, 0.0f, 3.0f) };
      auto lightDirNormalized = makeReactiveValue([](glm::vec3 lightDir){
         return glm::normalize(lightDir);
      }, lightDirection);
      // attenuation
      ObservableValue lightPos {glm::vec3(3.0f, 0.0f, -3.0f)};
      ObservableValue lightScale {1.0f};
      ...
      ObservableValue maxDistance {100};
      ...
      for(int i = 0; i < 10; i++){
         ...
         auto commonCubeSetter = [&](DrawUnit& cube){
            ...
            cube.addUniform("projection", projectionCoord.projection.get());
         };
      ...
      light.addUniform("projection", projectionCoord.projection.get());
      ...
      ObservableValue position { cubePositions[5] };
      ObservableValue axios { glm::vec3(1.0f, 0.3f, 0.5f) };
      ObservableValue angle { 40.0f };
      ...
      ObservableValue isSetterCamera { true };
      ObservableValue isSetterSelf { false };
      ...
      auto guiDrawer = [&]{
         ...
         if(typeVal == LightType::ATTENUATION){
            ...
            outerCutOffDegree.mayNotify();
            innerCutOffDegree.mayNotify();
         }
         ...
         isSetterCamera.mayNotify();
         ImGui::Checkbox("isSetterSelf", &isSetterSelf.val());
         isSetterSelf.mayNotify();
         ImGui::End();
      };
